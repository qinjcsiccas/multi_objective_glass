#!/usr/bin/env python3
"""MGEA - 无机玻璃多目标性能预测与筛选 | Streamlit App"""
...
st.sidebar.markdown("**碱土金属**")
c1, c2, c3, c4 = st.sidebar.columns(4)
with c1:
    mg = st.number_input(
        "Mg %", 0.0, 100.0, 0.0, 0.1, key="elem_Mg"
    )
with c2:
    ca = st.number_input(
        "Ca %", 0.0, 100.0, 0.0, 0.1, key="elem_Ca"
    )
with c3:
    sr = st.number_input(
        "Sr %", 0.0, 100.0, 0.0, 0.1, key="elem_Sr"
    )
with c4:
    ba = st.number_input(
        "Ba %", 0.0, 100.0, 0.0, 0.1, key="elem_Ba"
    )

st.sidebar.markdown("**网络形成体**")
c1, c2, c3 = st.sidebar.columns(3)
with c1:
    b = st.number_input(
        "B %", 0.0, 100.0, 0.0, 0.1, key="elem_B"
    )
with c2:
    al = st.number_input(
        "Al %", 0.0, 100.0, 0.0, 0.1, key="elem_Al"
    )
with c3:
    si = st.number_input(
        "Si %", 0.0, 100.0, 0.0, 0.1, key="elem_Si"
    )

st.sidebar.markdown("**过渡金属 / 其他**")
c1, c2, c3, c4 = st.sidebar.columns(4)
with c1:
    ti = st.number_input(
        "Ti %", 0.0, 100.0, 0.0, 0.1, key="elem_Ti"
    )
with c2:
    fe = st.number_input(
        "Fe %", 0.0, 100.0, 0.0, 0.1, key="elem_Fe"
    )
with c3:
    zr = st.number_input(
        "Zr %", 0.0, 100.0, 0.0, 0.1, key="elem_Zr"
    )
with c4:
    pb = st.number_input(
        "Pb %", 0.0, 100.0, 0.0, 0.1, key="elem_Pb"
    )
c1, c2 = st.sidebar.columns(2)
with c1:
    p = st.number_input(
        "P %", 0.0, 100.0, 0.0, 0.1, key="elem_P"
    )
with c2:
    f_val = st.number_input(
        "F %", 0.0, 100.0, 0.0, 0.1, key="elem_F"
    )

st.sidebar.markdown("**氧模式**")
manual_o = st.sidebar.toggle("手动输入 O", value=False, help="关闭则自动按电荷中性计算")
...
if manual_o:
    ...
    if not is_neutral and delta > 0.01:
        st.sidebar.warning(f"电荷不中性！正确 O = {correct_o:.2f}（偏差 {delta:.2f}）")
        if st.sidebar.button("✔️ 自动校正 O", width="stretch"):
    ...
    st.sidebar.caption("O 由电荷中性自动计算")
...
# Tabs
# ================================================================
tab1, tab2 = st.tabs(["📊 单点性能预测", "🧬 多目标遗传算法优化"])
...
with tab1:
    ...
    if st.button("🔮 预测性能", type="primary", width="stretch"):
        with st.spinner("预测中 (R 模型推理)... 约需 5-15 秒"):
            resp = predict_composition(raw_comp_pred)
        if "error" in resp:
            st.error(resp["error"])
        elif len(resp) < 5:
            st.warning("R 服务器尚未就绪，请重试")
        else:
    ...
    if "history" in st.session_state and st.session_state["history"]:
        ...
        st.download_button("📥 下载历史 CSV", csv, "mgea_history.csv", "text/csv")

# ===== Tab 2: GA Optimization =====
with tab2:
    st.subheader("多目标遗传算法优化")

    # Optimization targets
    with st.expander("🎯 优化目标与权重", expanded=True):
        st.caption("设定每个性能的优化方向和权重（设 0 则不参与优化）")
        cols = st.columns(5)
        ga_dir = {}
        ga_wt = {}
    ...
    # GA Parameters
    with st.expander("⚙️ GA 参数与搜索范围", expanded=True):
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            pop_size = st.number_input("种群大小", 5, 200, 30, 5)
        with c2:
            iters = st.number_input("迭代代数", 5, 100, 20, 5)
        with c3:
            mut = st.number_input("突变率", 0.01, 0.3, 0.05, 0.01)
        with c4:
            ga_boot = st.number_input("Bootstrap", 10, 1000, 100, 10)

        st.caption("元素搜索范围 (min ~ max, 0 = 禁用)")
        ga_mins = {}
        ga_maxs = {}
        elist = [
            "Li",
        ...
        if st.button("🗑️ 一键清零", width="stretch"):
    ...
    if st.button("🚀 开始多目标优化", type="primary", width="stretch"):
        if all(v <= 0 for v in ga_maxs.values()):
            st.error("请至少启用一种元素 (max > 0)")
        elif any(ga_mins[e] > ga_maxs[e] for e in elist):
            st.error("某些元素 min > max，请修正")
        else:
            with st.spinner("遗传算法优化中... 请耐心等待"):
                ga_params = {
                    ...
                    "dir": {
                        pk: ("min" if "越小" in ga_dir[pk] else "max")
                        for pk in PROP_KEYS
                    },
            ...
            if "error" in ga_resp:
                st.error(f"GA 失败: {ga_resp['error']}")
                if "_r_output" in ga_resp:
                    with st.expander("R GA 详细输出"):
    ...
    if "ga_results" in st.session_state and st.session_state["ga_results"]:
        ...
        top_n = st.number_input("显示前 N 个", 1, max(1, len(df)), min(20, len(df)), 5)
        ...
        if plot_cols:
            ...
            fig.update_xaxes(title_text="排名", row=len(plot_cols), col=1)
            st.plotly_chart(fig, width="stretch")

        # Best candidates table
        st.subheader("最优候选明细")
        dir_ga = {pk: ("min" if "越小" in ga_dir[pk] else "max") for pk in PROP_KEYS}
        best_rows = []
        ...
        for pk in PROP_KEYS:
            ...
            row = {
                "类别": f"{PROP_LABELS[pk]} {'↓' if dir_ga[pk] == 'min' else '↑'}最优 (No.{idx + 1})"
            }
            row.update(
